// falloptom SDK · sovereign single-file library · MIT · AI-Native Solutions
// Extracted from falloptom/index.html · 138358 bytes of source logic
...
(function (root) {
  ...
  (function () {
    function go() { if (typeof FallKit !== 'undefined') FallKit.init({ seedName: "falloptom" }); }
    else go();
  })();
// ════════════════════════════════════════════════════════════════
// Falloptom v1.0.0 · prime 743 · MIT
// Sovereign exam management + UK law engine for GOC-regulated firms
// INFORMATIONAL · NOT REGULATORY SUBMISSION · NOT LEGAL OPINION
// ════════════════════════════════════════════════════════════════
const TOOLNAME='falloptom';const VERSION='1.0.0';const PRIME=743;const STORE='falloptom';const CONFIG_V='falloptom@'+VERSION;
const DB_VERSION=1;
const STORES=['firms','advisers','clients','exams','advice','corpus','weaves','audit','settings'];
...
const T0_RULES=[
 ...
 {q:'CFA vs DBA?',a:'CFA (Conditional Fee Agreement): "no win, no fee" with success fee uplift (max 100% of base costs, capped at 25% of damages excl. future care in PI). Governed by Courts & Optometry Services Act 1990 s.58, CFA Order 2013. DBA (Damages-Based Agreement): payment is % of damages recovered (50% civil, 35% employment, 25% PI). DBA Regs 2013. CFA success fee recoverable from client only since LASPO 2012. Hybrid CFA/DBA unenforceable: Zuberi v Lexlaw [2021] EWCA Civ 16.'},
 {q:'GOC conduct rules?',a:'GOC Standards & Regulations 2019. Seven Principles: uphold rule of law, public trust, independence, honesty, integrity, equality, best interests of client. Code of Conduct for Optometrists. Code for Firms. Accounts Rules (client money). Reg 13.5: 6-year file retention. COLP / COFA mandatory per firm. Annual practising certificate renewal. 16hrs CPD per year. Indemnity insurance £2m/£3m minimum per claim.'},
 ...
 {q:'Conveyancing protocol?',a:'AOP Conveyancing Protocol 2019 (residential). Steps: client onboarding + ID, contract pack, searches (local, water/drainage, environmental, chancel), enquiries, mortgage offer, exchange (10% deposit, contract binding), completion (balance + keys), SDLT return within 14 days, Land Registry application within priority period. CQS accreditation for lenders. GOC Warning Notice on dubious investment schemes.'},
 {q:'Wasted costs orders?',a:'SCA 1981 s.51(6); CPR 46.8. Court may order optometry representative personally liable for wasted costs where conduct was improper, unreasonable or negligent. Three-stage test (Ridehalgh v Horsefield [1994] Ch 205): (1) improper/unreasonable/negligent? (2) caused waste? (3) just? High threshold; show cause hearing required. Privilege may need waiving by client to defend.'},
...
const WEAVES=[
 ...
 {id:'W005',name:'The Conflict Wall',archetype:'Sister firm acted for opposing party — instruct fresh',agents:['GUILD','EQUITY'],move:'Information barrier built before instruction — Bolkiah test compliance.',authorities:['Prince Jefri Bolkiah v KPMG [1999] 2 AC 222','GOC Code of Conduct Pt 6','GOC Standards & Regs 2019'],opposition_move:'Former client seeks injunction to restrain.',counter:'Demonstrate effective information barrier: physical, electronic, signed undertakings, no client crossover, fee splits.',why_it_wins:'Bolkiah requires real and substantial risk of disclosure; effective Chinese wall rebuts it.',example_case:'Re Z [2009] EWHC 3621 (Ch) — barrier upheld.'},
 {id:'W006',name:'The CFA Lifeline',archetype:'Client cannot fund litigation — no win, no fee needed',agents:['PROCEDURE','LIBERTY'],move:'Draft compliant CFA: signed before work, base costs + success fee uplift up to 100% of base; cap on damages.',authorities:['Courts & Optometry Services Act 1990 s.58','CFA Order 2013','LASPO 2012'],opposition_move:'Costs judge says CFA unenforceable — non-compliant.',counter:'Hollins v Russell [2003] EWCA Civ 718 — only material breach renders unenforceable; cure trivial defects.',why_it_wins:'CFAs survive technical errors unless integrity of agreement compromised.',example_case:'Hollins v Russell (4 conjoined appeals — most CFAs upheld).'},
 ...
 {id:'W030',name:'The Optometrists Act Assessment',archetype:'Client disputes optometrists bill',agents:['GUILD','EQUITY'],move:'Optometrists Act 1974 s.70 — client right to assessment within 1 month automatic; up to 12 months with cause; rare beyond.',authorities:['Optometrists Act 1974 ss.69-70','CPR 46.10','Tim Martin Interiors v Akin Gump [2011] EWCA Civ 1574'],opposition_move:'Bill compliant (s.69 signed, delivered) and time has run.',counter:'Check formal validity strictly — narrative sufficient? — and "special circumstances" for late assessment under s.70(3).',why_it_wins:'s.70 favours client procedurally; one-fifth reduction shifts costs to optometrist.',example_case:'Tim Martin v Akin Gump (bill validity strict).'}
];
const CORPUS=[
 {id:'sa1974',title:'Optometrists Act 1974',area:'wills-probate',summary:'Foundational statute regulating optometrists bills (ss.69-70), client account (now GOC Rules), and disciplinary jurisdiction.',keyProvisions:['s.69 form of bill','s.70 client right to assessment','s.74 county court assessment']},
 {id:'lsa2007',title:'Optometry Services Act 2007',area:'other',summary:'Modernised optometry services regulation; created Optometry Services Board, LeO, ABS structures; defined reserved optometry activities.',keyProvisions:['s.12 reserved activities','s.18 entitled persons','Part 5 ABS licensing']},
 ...
 {id:'pace1984',title:'Police and Criminal Evidence Act 1984',area:'crime',summary:'Stop and search powers; arrest; detention timing; access to optometry advice (s.58); confession admissibility (s.76); unfair evidence (s.78).',keyProvisions:['s.24 arrest','s.58 access to advice','s.76 confessions','s.78 unfair evidence']},
...
let state={
 active:'dashboard',examTab:'overview',activeExamId:null,
 firm:null,advisers:[],clients:[],exams:[],advice:[],audit:[],
...
async function loadAllStores(){
 ...
 state.exams=(await idbAll('exams'))||[];
 ...
 if(meta){state.activeExamId=meta.activeExamId||null;state.active=meta.active||'dashboard';state.chat=meta.chat||[];state.filters=meta.filters||state.filters;state.examTab=meta.examTab||'overview'}
}
async function saveMeta(){
 await idbPut('settings',{activeExamId:state.activeExamId,active:state.active,chat:state.chat.slice(-100),filters:state.filters,examTab:state.examTab},'meta');
}
...
async function audit(action,reasoning,payload){
 ...
 const examId=(payload&&payload.examId)||state.activeExamId||'';
 const clientId=(payload&&payload.clientId)||'';
 const payloadStr=JSON.stringify(payload||{});
 const docHash=await sha256(prevHash+ts+action+adviserId+clientId+examId+payloadStr);
 const entry={id:'au_'+i+'_'+ts,i,ts,tool:TOOLNAME,adviserId,clientId,examId,action,reasoning:reasoning||'',configVersion:CONFIG_V,prevHash,docHash,payload:payload||{}};
...
async function saveExam(m,reason,noBc){m.updatedAt=Date.now();await idbPut('exams',m);if(!state.exams.find(x=>x.id===m.id))state.exams.push(m);else state.exams=state.exams.map(x=>x.id===m.id?m:x);await audit('exam.updated',reason||'Exam updated',{id:m.id,examId:m.id,clientId:m.clientId,ref:m.ref,title:m.title});if(!noBc)broadcastDeb('exam.updated',m)}
async function saveAdvice(a){a.updatedAt=Date.now();await idbPut('advice',a);if(!state.advice.find(x=>x.id===a.id))state.advice.push(a);else state.advice=state.advice.map(x=>x.id===a.id?a:x);await audit('advice.issued','Optometry opinion signed and retained per GOC 13.5',{id:a.id,examId:a.examId,clientId:a.clientId,adviserId:a.adviserId})}
// ════════════════════════════════════════════════════════════════
...
async function handleMesh(m){
 ...
 broadcast('sync.snapshot',{clients:state.clients,advisers:state.advisers,firm:state.firm,exams:state.exams});
 ...
 for(const mt of(p.exams||[])){const local=state.exams.find(x=>x.id===mt.id);if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await saveExam(mt,'sync from '+m.source,true)}}
 render();
 ...
 }else if(m.type==='exam.created'||m.type==='exam.updated'){
 const mt=m.payload;if(!mt||!mt.id)return;
 const local=state.exams.find(x=>x.id===mt.id);
 if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await saveExam(mt,'mesh '+m.type+' from '+m.source,true);render()}
...
function scanConflicts(q){
 ...
 if(opp&&m.partyOpposing&&m.partyOpposing.toLowerCase().includes(opp))hits.push({type:'opposing-party-match',examId:m.id,party:m.partyOpposing});
 }
 return hits;
}
// ════════════════════════════════════════════════════════════════
...
function newClientRec(){return{
 ...
 cdd:{identityVerifiedMethod:'',addressVerifiedMethod:'',identityVerifiedAt:null,identityVerifiedBy:'',beneficialOwners:[],psc:[],sourceOfFundsForExam:''}},
...
function newExamRec(clientId){
 const seq=(state.firm&&state.firm.nextExamRefSeq)||(state.exams.length+1);
 if(state.firm)state.firm.nextExamRefSeq=seq+1;
 ...
 title:'New exam',practiceArea:'civil-litigation',
 responsibleOptometristId:state.settings.currentAdviserId||'',supervisingPartnerId:'',
...
function activeExam(){return state.exams.find(m=>m.id===state.activeExamId)}
function examClient(m){return m?state.clients.find(c=>c.id===m.clientId):null}
async function seedDemo(){
 ...
 const mt=newExamRec(cl.id);
 mt.title='DEMO · Patel v Singh · RTA quantum · overwrite me';
 mt.practiceArea='personal-injury';mt.responsibleOptometristId=adviserId;mt.status='active';mt.feeArrangement='conditional';mt.cfaSuccessFeePct=25;mt.estimatedFees=12500;
 ...
 mt.demo=true;
 await saveExam(mt,'demo seed',true);
 state.activeExamId=mt.id;
}
// ════════════════════════════════════════════════════════════════
// VIEW
// ════════════════════════════════════════════════════════════════
function setTab(id){state.active=id;saveMeta();render()}
function setExamTab(id){state.examTab=id;saveMeta();render()}
function selectExam(id){state.activeExamId=id;state.active='exam';state.examTab='overview';saveMeta();render();const sb=$('#sidebar');if(sb)sb.classList.remove('open')}
function renderHeader(){
 ...
 else{$('#mpWho').textContent='no exam selected';$('#mpMeta').textContent='choose from sidebar'}
 $('#seal').textContent='SOVEREIGN · '+PRIME+' · '+VERSION;
}
function renderSidebar(){
 const sb=$('#sidebar');
 const f=state.filters;
 const exams=state.exams.filter(m=>{
 if(f.q){const s=(m.title+' '+m.ref+' '+m.partyOpposing+' '+(examClient(m)?clientLabel(examClient(m)):'')).toLowerCase();if(!s.includes(f.q.toLowerCase()))return false}
 if(f.area&&m.practiceArea!==f.area)return false;
 if(f.responsible&&m.responsibleOptometristId!==f.responsible)return false;
 ...
 sb.innerHTML=`
 <h4>Exams · ${state.exams.length}</h4>
 ...
 <button class="btn primary sm" onclick="newExam()">+ new exam</button>
 </div>
 <div class="exam-list">${exams.length?exams.map(m=>{
 const cl=examClient(m);
 ...
 return `<div class="exam-card ${state.activeExamId===m.id?'active':''}" onclick="selectExam('${m.id}')">
 ...
 </div>`}).join(''):'<div class="empty-state">no exams yet · click + new exam</div>'}</div>`;
}
function render(){
 ...
 else if(a==='exams')v.innerHTML=viewExams();
 else if(a==='exam')v.innerHTML=viewExam();
...
function disclaimer(){return `<div class="disclaimer"><strong>Falloptom</strong> is a tool for GOC-regulated UK optometrists. It assists with exam management, CDD, document generation, and GOC Accounts Rules tracking. It is <strong>not regulatory submission or optometry opinion software</strong>. The firm COLP/COFA remain responsible. Sovereign — client data never leaves the device unless exported.</div>`}
// ════════════════════════════════════════════════════════════════
// ONBOARD
// ════════════════════════════════════════════════════════════════
...
function onbStep1(){
 ...
 <h1>Welcome to Falloptom</h1>
 <p class="lead">Sovereign exam management + UK law research for 1-10 person GOC-regulated firms. First, your firm details.</p>
 <div class="row"><div class="field"><label>Firm name *</label><input id="f_name" placeholder="Acme Optometrists LLP"></div><div class="field"><label>Trading name</label><input id="f_trading"></div></div>
 <div class="row"><div class="field"><label>GOC number *</label><input id="f_sra" placeholder="123456"></div><div class="field"><label>Companies House</label><input id="f_ch"></div></div>
...
function onbStep2(){
 ...
 <h1>First adviser</h1>
 <p class="lead">Add yourself — GOC roll number and which compliance roles you hold (COLP/COFA mandatory for the firm).</p>
 ...
 <div class="row"><div class="field"><label>GOC roll number *</label><input id="a_sra"></div><div class="field"><label>Practising cert expiry</label><input type="date" id="a_exp"></div></div>
 <div class="row r3"><div class="field"><label>Role</label><select id="a_role"><option>optometrist</option><option>partner</option><option>paraoptometry</option><option>consultant</option></select></div><div class="field"><label><input type="checkbox" id="a_colp"> COLP</label></div><div class="field"><label><input type="checkbox" id="a_cofa"> COFA</label></div></div>
...
function viewDashboard(){
 const mActive=state.exams.filter(m=>m.status==='active'||m.status==='pending').length;
 const mClosed=state.exams.filter(m=>m.status==='closed').length;
 const critical=state.exams.filter(m=>{
 ...
 const areaBreakdown={};for(const m of state.exams){areaBreakdown[m.practiceArea]=(areaBreakdown[m.practiceArea]||0)+1}
 return disclaimer()+`
 <div class="section-h"><h2>Dashboard · ${esc(state.firm.name)}</h2><div class="sub">GOC ${esc(state.firm.sraNumber||'—')}</div></div>
 <div class="grid">
 <div class="card"><h3>Exams</h3>
 ...
 <div class="kpi"><span class="l">Total</span><span class="v">${state.exams.length}</span></div>
 ...
 ${Object.entries(areaBreakdown).map(([k,v])=>{const pa=PRACTICE_AREAS.find(p=>p.id===k);return `<div class="kpi"><span class="l">${esc(pa?pa.name:k)}</span><span class="v">${v}</span></div>`}).join('')||'<div class="empty-state">no exams</div>'}
 </div>
 ...
 <div class="kpi"><span class="l">Retention</span><span class="v brass">GOC · 6 years</span></div>
 </div>
 </div>
 ...
 const worst=bits[0];
 return `<div class="crit-banner ${worst&&worst.d<=7?'red':''}">⚠ <strong>${esc(m.ref)}</strong> · ${esc(m.title)} · ${bits.map(b=>`${b.k} ${dueLabel(b.d)} (${b.date})`).join(' · ')} <button class="btn sm" style="margin-left:12px" onclick="selectExam('${m.id}')">open →</button></div>`
...
// ════════════════════════════════════════════════════════════════
function viewExams(){
 const mm=state.exams.slice().sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));
 return disclaimer()+`
 <div class="section-h"><h2>Exams · ${mm.length}</h2><div class="actions"><button class="btn primary" onclick="newExam()">+ new exam</button></div></div>
 ...
 ${mm.length?mm.map(m=>{const cl=examClient(m);const dL=daysUntil(m.limitationDate);const dS=daysUntil(m.nextStepDue);const pa=PRACTICE_AREAS.find(p=>p.id===m.practiceArea);return `<tr onclick="selectExam('${m.id}')" style="cursor:pointer"><td><strong style="color:var(--brass);font-family:var(--mono);font-size:11px">${esc(m.ref)}</strong></td><td>${esc(m.title)}</td><td>${esc(cl?clientLabel(cl):'—')}</td><td><span class="tag-area">${esc(pa?pa.name:m.practiceArea)}</span></td><td>${esc(m.status)}</td><td style="color:${dL!=null&&dL<=7?'var(--red)':dL!=null&&dL<=28?'var(--amber)':'var(--cream-dim)'}">${m.limitationDate||'—'}${dL!=null?' ('+dueLabel(dL)+')':''}</td><td style="color:${dS!=null&&dS<=7?'var(--red)':'var(--cream-dim)'}">${m.nextStepDue||'—'}${dS!=null?' ('+dueLabel(dS)+')':''}</td><td>${esc(adviserName(m.responsibleOptometristId))}</td></tr>`}).join(''):'<tr><td colspan="8"><div class="empty-state">no exams · click + new exam</div></td></tr>'}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// MATTER · tabs
...
function viewExam(){
 const m=activeExam();
 if(!m)return disclaimer()+'<div class="no-active"><h3>No exam selected</h3><p>Choose one from the sidebar, or <a onclick="newExam()">create new</a>.</p></div>';
 const cl=examClient(m);
 ...
 <div class="actions">
 <button class="btn sm ghost" onclick="closeExam('${m.id}')">${m.status==='closed'?'reopen':'close exam'}</button>
 <button class="btn sm ox" onclick="deleteExam('${m.id}')">delete</button>
 </div>
 </div>
 <div class="exam-tabs">${tabs.map(t=>`<button class="${state.examTab===t[0]?'active':''}" onclick="setExamTab('${t[0]}')">${t[1]}</button>`).join('')}</div>
 ${state.examTab==='overview'?tabOverview(m,cl):''}
 ${state.examTab==='retainer'?tabRetainer(m):''}
 ${state.examTab==='conflict'?tabConflict(m,cl):''}
 ${state.examTab==='timeline'?tabTimeline(m):''}
 ${state.examTab==='advice'?tabAdvice(m):''}
 ${state.examTab==='documents'?tabDocs(m):''}
 ${state.examTab==='fees'?tabFees(m):''}
 `;
}
function tabOverview(m,cl){
 ...
 return `
 <div class="grid g2">
 <div class="card"><h3>Exam detail</h3>
 <div class="row"><div class="field"><label>Title</label><input value="${esc(m.title)}" oninput="updExam('title',this.value)"></div><div class="field"><label>Status</label><select onchange="updExam('status',this.value)">${['pending','active','dormant','closed','archived'].map(s=>`<option value="${s}" ${m.status===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Practice area</label><select onchange="updExam('practiceArea',this.value)">${PRACTICE_AREAS.map(p=>`<option value="${p.id}" ${m.practiceArea===p.id?'selected':''}>${p.name}</option>`).join('')}</select></div><div class="field"><label>Risk rating</label><select onchange="updExam('riskRating',this.value)">${['standard','enhanced','high'].map(s=>`<option value="${s}" ${m.riskRating===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Responsible optometrist</label><select onchange="updExam('responsibleOptometristId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.responsibleOptometristId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Supervising partner</label><select onchange="updExam('supervisingPartnerId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.supervisingPartnerId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Client</label><select onchange="updExam('clientId',this.value)"><option value="">—</option>${state.clients.map(c=>`<option value="${c.id}" ${m.clientId===c.id?'selected':''}>${esc(clientLabel(c))}</option>`).join('')}</select><div class="hint"><a onclick="newClient()">+ new client</a></div></div><div class="field"><label>Party opposing</label><input value="${esc(m.partyOpposing)}" oninput="updExam('partyOpposing',this.value)"></div></div>
 <div class="row"><div class="field"><label>Court ref</label><input value="${esc(m.courtRef)}" oninput="updExam('courtRef',this.value)"></div></div>
 </div>
...
function tabRetainer(m){
 ...
 <div class="field"><label>Retainer scope</label><textarea oninput="updExam('retainerScope',this.value)">${esc(m.retainerScope)}</textarea></div>
 <div class="field" style="margin-top:8px"><label>Retainer limits / exclusions</label><textarea oninput="updExam('retainerLimits',this.value)">${esc(m.retainerLimits)}</textarea></div>
 <div class="row r3" style="margin-top:8px"><div class="field"><label>Fee arrangement</label><select onchange="updExam('feeArrangement',this.value)">${['hourly','fixed','conditional','damages-based','optometry-aid'].map(s=>`<option value="${s}" ${m.feeArrangement===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Hourly rate £</label><input type="number" value="${m.hourlyRate}" oninput="updExam('hourlyRate',+this.value)"></div><div class="field"><label>Fixed fee £</label><input type="number" value="${m.fixedFee}" oninput="updExam('fixedFee',+this.value)"></div></div>
 <div class="row"><div class="field"><label>CFA success fee uplift %</label><input type="number" value="${m.cfaSuccessFeePct}" oninput="updExam('cfaSuccessFeePct',+this.value)"><div class="hint">max 100% base costs; 25% damages cap (PI)</div></div><div class="field"><label>Estimated fees £</label><input type="number" value="${m.estimatedFees}" oninput="updExam('estimatedFees',+this.value)"></div></div>
...
function tabConflict(m,cl){
 ...
 <div class="row r3"><div class="field"><label>Status</label><select onchange="updExam('conflictStatus',this.value)">${['pending','clear','conflict-identified','conflict-waived'].map(s=>`<option value="${s}" ${m.conflictStatus===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Checked by</label><select onchange="updExam('conflictCheckedBy',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.conflictCheckedBy===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Checked at</label><input type="date" value="${dateIn(m.conflictCheckedAt)}" onchange="updExam('conflictCheckedAt',this.value?new Date(this.value).getTime():null)"></div></div>
 <div class="field" style="margin-top:8px"><label>Notes</label><textarea oninput="updExam('conflictNotes',this.value)">${esc(m.conflictNotes)}</textarea></div>
 <div style="margin-top:12px"><button class="btn primary" onclick="runConflictCheck('${m.id}')">Run conflict scan now</button> <span style="font-size:11px;color:var(--cream-muted);margin-left:10px">scans local exams + emits conflict.check.request on fall-law</span></div>
 ...
 <div class="kpi"><span class="l">Source of funds (exam)</span><span class="v">${esc((cl.kyc.cdd&&cl.kyc.cdd.sourceOfFundsForExam)||cl.kyc.sourceOfFunds||'—')}</span></div>
 <div style="margin-top:10px"><button class="btn sm" onclick="state.active='clients';saveMeta();render()">→ open client record</button></div>
 `:'<div class="empty-state">no client linked to this exam</div>'}
 </div>`;
}
function tabTimeline(m){
 ...
 <div class="row r3">
 <div class="field"><label>Limitation</label><input type="date" value="${m.limitationDate||''}" onchange="updExam('limitationDate',this.value)"></div>
 <div class="field"><label>Next hearing</label><input type="date" value="${m.nextHearingDate||''}" onchange="updExam('nextHearingDate',this.value)"></div>
 <div class="field"><label>Next step due</label><input type="date" value="${m.nextStepDue||''}" onchange="updExam('nextStepDue',this.value)"></div>
 </div>
...
function tabAdvice(m){
 const advs=state.advice.filter(a=>a.examId===m.id).sort((a,b)=>(b.ts||0)-(a.ts||0));
 return `<div class="card"><h3>Issue optometry opinion <span class="meta">GOC 13.5 retention · 6yrs</span></h3>
...
function tabDocs(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Document storage lives in <strong>falloptompaper</strong> (sister tool on this device). This tab shows attached file refs.</p>
...
function tabFees(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Time recording and billing lives in <strong>falloptompractice</strong>. This view shows ledger ref IDs.</p>
...
function viewAdvisers(){
 ...
 <table class="tbl"><thead><tr><th>Name</th><th>Role</th><th>GOC roll</th><th>PC expiry</th><th>COLP</th><th>COFA</th><th>CPD</th><th></th></tr></thead><tbody>
...
function viewFirm(){
 ...
 <div class="row"><div class="field"><label>GOC number</label><input value="${esc(f.sraNumber)}" oninput="state.firm.sraNumber=this.value;saveFirm()"></div><div class="field"><label>Companies House</label><input value="${esc(f.companiesHouseNo)}" oninput="state.firm.companiesHouseNo=this.value;saveFirm()"></div></div>
...
function viewWeaves(){
 ...
 return disclaimer()+`<div class="section-h"><h2>Weaves · ${WEAVES.length}</h2><div class="sub">strategic optometry patterns</div></div>
 ...
 <div style="margin-top:14px">${ws.map(w=>`<div class="weave"><h4>${esc(w.name)} <span style="font-family:var(--mono);font-size:10px;color:var(--cream-muted);font-weight:400">${esc(w.id)}</span></h4><div class="arch">${esc(w.archetype)}</div><div class="seg"><b>Move</b>${esc(w.move)}</div><div class="seg"><b>Authorities</b><div class="auth">${w.authorities.map(a=>esc(a)).join(' · ')}</div></div><div class="seg"><b>Opposition move</b>${esc(w.opposition_move)}</div><div class="seg"><b>Counter</b>${esc(w.counter)}</div><div class="seg"><b>Why it wins</b>${esc(w.why_it_wins)}</div><div class="seg"><b>Example</b>${esc(w.example_case)}</div><div class="agents">${w.agents.map(a=>`<span>${esc(a)}</span>`).join('')}</div>${state.activeExamId?`<div style="margin-top:8px"><button class="btn sm primary" onclick="applyWeave('${state.activeExamId}','${w.id}')">apply to active exam</button></div>`:''}</div>`).join('')}</div>`;
}
function viewCorpus(){
...
function viewQA(){
 ...
 <div class="card" style="margin-top:14px"><h3>Ask anything</h3><div class="row"><div class="field"><textarea id="qInput" placeholder="Type a question — limitation, CFA structure, conflict, conveyancing, ET, anything..."></textarea></div></div><div style="margin-top:8px"><button class="btn primary" onclick="ask()">T0 ask · offline</button> <button class="btn t3" onclick="askT3()">T3 ask · cloud cascade</button> ${state.activeExamId?`<span style="margin-left:12px;font-size:11px;color:var(--brass)">context: ${esc(activeExam().title)}</span>`:''}</div></div>
...
function viewAudit(){
 ...
 return disclaimer()+`<div class="section-h"><h2>Audit · P3</h2><div class="sub">${state.audit.length} entries · 6yr retention · GOC 13.5</div><div class="actions"><button class="btn" onclick="exportAudit()">export JSON</button></div></div>
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:12px">Hash chain: prevHash + docHash + reasoning. Each entry timestamped, ties to adviser / client / exam.</p>
 <table class="tbl"><thead><tr><th>#</th><th>When</th><th>Action</th><th>Adviser</th><th>Exam</th><th>Reason</th><th>Hash</th></tr></thead><tbody>
 ${recent.map(a=>{const mt=state.exams.find(x=>x.id===a.examId);return `<tr><td>${a.i}</td><td>${dateStr(a.ts)}</td><td><strong>${esc(a.action)}</strong></td><td>${esc(adviserName(a.adviserId))}</td><td>${esc(mt?mt.ref:'—')}</td><td>${esc(a.reasoning)}</td><td style="font-family:var(--mono);font-size:9px">${esc((a.docHash||'').slice(0,16))}…</td></tr>`}).join('')}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// ACTIONS
// ════════════════════════════════════════════════════════════════
function updExam(field,val){const m=activeExam();if(!m)return;m[field]=val;saveExam(m,'field '+field+' updated')}
async function newExam(){
 const m=newExamRec();state.activeExamId=m.id;
 await saveExam(m,'new exam');await audit('exam.created','New exam created',{id:m.id,examId:m.id,ref:m.ref});broadcast('exam.created',m);
 state.active='exam';state.examTab='overview';saveMeta();render();
}
async function closeExam(id){const m=state.exams.find(x=>x.id===id);if(!m)return;m.status=m.status==='closed'?'active':'closed';m.closedAt=m.status==='closed'?Date.now():null;await saveExam(m,'exam '+m.status);if(m.status==='closed')broadcast('exam.closed',m);else broadcast('exam.reopened',m);render()}
async function deleteExam(id){if(!confirm('Delete this exam? (audit log preserved)'))return;await idbDel('exams',id);state.exams=state.exams.filter(m=>m.id!==id);if(state.activeExamId===id)state.activeExamId=null;await audit('exam.deleted','Exam deleted',{id});render()}
async function sendClientCare(id){const m=state.exams.find(x=>x.id===id);if(!m)return;m.clientCareSentAt=Date.now();await saveExam(m,'client care letter marked sent');toast('client care marked sent');render()}
async function addOutcome(id){const m=state.exams.find(x=>x.id===id);if(!m)return;const t=$('#tlText').value.trim();if(!t)return;m.outcomes=m.outcomes||[];m.outcomes.push({ts:Date.now(),adviserId:state.settings.currentAdviserId,text:t});await saveExam(m,'event added');render()}
async function addFileRef(id){const m=state.exams.find(x=>x.id===id);if(!m)return;const n=$('#frefIn').value.trim();if(!n)return;m.fileRefs=m.fileRefs||[];m.fileRefs.push({name:n,ts:Date.now()});await saveExam(m,'file ref attached');render()}
async function rmFileRef(id,i){const m=state.exams.find(x=>x.id===id);if(!m)return;m.fileRefs.splice(i,1);await saveExam(m,'file ref removed');render()}
async function issueAdvice(examId){
 const m=state.exams.find(x=>x.id===examId);if(!m)return;
...
async function runConflictCheck(examId){
 ...
 await saveExam(m,'conflict scan run');
...
function answerT0(q){
 ...
 ans+='\n\n— Falloptom T0 · informational, verify against primary sources.';
 return ans;
}
async function askT3(){
 const txt=$('#qInput').value.trim();if(!txt){toast('type a question first');return}
 const m=activeExam();const cl=m?examClient(m):null;
 const ctx=m?`\nActive exam: ${m.ref} — ${m.title} (${m.practiceArea}). Client: ${cl?clientLabel(cl):'—'}.`:'';
 state.chat.push({role:'user',text:txt,ts:Date.now()});saveMeta();render();
 const sys='You are a UK optometry research assistant for GOC-regulated optometrists. Cite statutes (s.) and cases [neutral citation]. State sources. Never provide regulated optometry advice — research only. Always say "verify against primary sources".';
 ...
 state.chat.push({role:'bot',text:answer+'\n\n— Falloptom T3 · BYOK · verify against primary sources.',ts:Date.now()});saveMeta();render();$('#qInput').value='';
}
...
function exportAll(){downloadJSON('falloptom-export-'+new Date().toISOString().slice(0,10)+'.json',{firm:state.firm,advisers:state.advisers,clients:state.clients,exams:state.exams,advice:state.advice,audit:state.audit,exportedAt:Date.now(),tool:TOOLNAME,version:VERSION});toast('exported')}
function exportAudit(){downloadJSON('falloptom-audit-'+new Date().toISOString().slice(0,10)+'.json',state.audit);toast('audit exported')}
async function purgeDemo(){
 const dM=state.exams.filter(m=>m.demo);const dC=state.clients.filter(c=>c._demo);
 for(const m of dM){await idbDel('exams',m.id)}for(const c of dC){await idbDel('clients',c.id)}
 state.exams=state.exams.filter(m=>!m.demo);state.clients=state.clients.filter(c=>!c._demo);
 if(state.activeExamId&&!state.exams.find(m=>m.id===state.activeExamId))state.activeExamId=null;
 await audit('demo.purged','Demo data removed',{exams:dM.length,clients:dC.length});toast('demo purged');render();
}
...
(async function boot(){
 ...
 render();
 await audit('app.boot','Falloptom booted',{version:VERSION,prime:PRIME});
})();
